...
An high quality tooling is a strong base of the Rust development environment. 
The integrated package manager \textbf{Cargo}, documentation generator \textbf{rustdoc} and built-in 
testing framework significantly enhance developer productivity. The Rust package registry, crates.io, 
is an important source of libraries for different applications.

Beside the growing usage of Rust, it still relies on interoperability with C/C++ ecosystem, 
specially for low-level applications (eg. hardware interactions, graphics, legacy protocols). 
Interfacing Rust with foreign code is possible using through the \gls{FFI}. Usualy, an \gls{FFI} is 
functional and efficient but often requires carefull design of abstractions, specially when crossing language 
boundaries with different memory management models.
...
It has hardware specific libraries, mature compilers, profiling and debugging tools and frameworks for 
...
Industry data confirms that memory-unsafe languages (primarily C and C++) account for the majority of 
...
Usualy an incremental operation includes three steps: the load of the variable into a register, 
the value increment at the register and the value store back at the memory.

When this operation is not protected by locking (e.g. using threads) or atomic types (\textbf{std::atomic<int>}), 
both threads
can load the same value from memory, increment it and store it, simultaneously. Since the initial value is the same, 
instead of expected increment by 2 it only will increment by 1.
...
This ubiquity creates a unique dichotomy during migration, because the same abstractions that enabled 
modern C++ design patterns  also amplify the complexity of introducing a Rust based ownership model 
...
\begin{itemize}
    ...
    are ill-suited to interface with Boost's header-only, template-heavy design: the absence of concrete, 
    non-templated symbols at the ABI level often forces developers to introduce intermediate C-compatible facades. 
    ...
    \item \textbf{Semantic opportunity}: Many Boost libraries were explicitly designed to encode disciplined 
    resource management 
    ...
    (for example, \verb|boost::shared_ptr<T>| $\rightarrow$ \verb|std::sync::Arc<T>|,
\verb|boost::optional<T>| $\rightarrow$ \verb|Option<T>|, or
\verb|boost::variant| $\rightarrow$ a Rust \texttt{enum}).
...
A best-case historical study of open-source vulnerabilities indicates that approximately 58.2\% of 
historical vulnerabilities 
in prominent C projects would have been virtually impossible in safe Rust. \parencite{meneely2025just} 

Code vulnerabilities are formally listed at \gls{CWE}. \gls{CWE} was community developed and list 
...
Hybrid architectures are structural design patterns that allow two languages to coexist in a single system, 
as systems with C++ and Rust. They share the same low-level control but have different compilation models and 
safety guarantees. Due to it the architecture should focus on isolating the boundary to minimize friction.
...
\begin{itemize}
    \item \textbf{FFI vulnerabilities}: Rust's safety guarantees (ownership, borrow checking) strictly end at 
    the Foreign Function Interface (FFI) boundary. Crossing this boundary destroys safety unless rigorously wrapped, 
    creating a safety air gap. Dynamic analysis tools like Rustcheck have been proposed to detect memory safety issues 
    specifically arising from unsafe Rust usage at these boundaries. \parencite{xia2023rustcheck}
    \item \textbf{Integration risks}: Passing complex Boost data structures across the FFI boundary introduces risks 
    of unsafe pointer usage, exception mismatches, and memory leaks.
    \item \textbf{Concurrency conflicts}: Bridging Boost.Asio or Boost.Thread with Rust's concurrency model can lead to 
    undefined behavior, specifically when distinct async runtimes or threading models attempt to interoperate 
...
\begin{itemize}
    \item \textbf{Performance overhead}: FFI boundaries introduce latency (10–20\% overhead in some cases) due to 
    data marshalling. In latency-sensitive domains (HPC, HFT), an ad hoc porting strategy that places FFI boundaries 
    in hot loops is unacceptable.
    \item \textbf{Missing methodology}: There is currently no standardized process for decomposing a Boost-heavy 
    application into portable units. Developers need a systematic way to identify architectural seams where the cost of 
    the FFI boundary is minimized and the safety value of Rust is maximized.
\end{itemize}

The central problem is not just the translation of code, but the architectural interoperability of two diverging 
memory models. 
...
\subsection{Evaluation Criteria for Interoperability Mechanisms}
\label{sec:ffi_evaluation_criteria}

The interoperability mechanisms discussed above are not equivalent and should not be selected solely on the basis of whether they can technically connect Rust and C++. For an incremental migration of legacy Boost components, the mechanism must also satisfy constraints imposed by the existing C++ standard, the desired safety boundary, the target platforms, the build system, and the complexity of the exposed interface.

The following criteria are therefore used to compare the mechanisms considered in this dissertation:

\begin{itemize}
    \item \textbf{C++ compatibility}: the minimum C++ language standard required by the interoperability mechanism and its ability to integrate with legacy C++03 code.
    \item \textbf{Safety}: the extent to which the mechanism can express interface contracts and reduce manually written FFI code, without assuming that the C++ implementation itself is memory-safe.
    \item \textbf{Portability}: dependencies on compiler versions, platforms, language standards, and generated artifacts.
    \item \textbf{Performance}: expected runtime and call-boundary overhead and the ability to expose low-level operations without unnecessary conversions.
    \item \textbf{ABI stability}: dependence on a C++ ABI versus a stable C-compatible boundary.
    \item \textbf{Template support}: ability to represent or wrap generic and template-heavy Boost interfaces.
    \item \textbf{Build complexity}: additional generators, headers, build-script integration, and maintenance requirements.
    \item \textbf{Auditability}: the ease with which the externally visible interface, ownership rules, and unsafe assumptions can be inspected and documented.
\end{itemize}

Table~\ref{tab:ffi_mechanism_comparison} summarizes the assessment. The values are qualitative suitability assessments for the migration scenario considered in this dissertation; they are not universal rankings of the technologies.
...
\hline
\textbf{Criterion} & \textbf{C ABI} & \textbf{cxx} & \textbf{bindgen} & \textbf{autocxx} & \textbf{cbindgen} \\ \hline
C++03 compatibility & High & Low & High & Low & High \\ \hline
Safety / interface checking & Medium & High & Medium & High & High \\ \hline
Portability & High & Medium & Medium & Medium & High \\ \hline
Performance transparency & High & High & High & Medium & High \\ \hline
ABI stability & High & Medium & Medium & Medium & High \\ \hline
Template-heavy C++ support & Low & Medium & High & High & N/A \\ \hline
Build complexity & Low & Medium & Medium & High & Low \\ \hline
Auditability & High & Medium & Medium & Medium & High \\ \hline
\end{tabularx}
\caption{Qualitative comparison of Rust/C++ interoperability mechanisms for the migration scenario}
\label{tab:ffi_mechanism_comparison}
\end{table}

The comparison highlights an important methodological point: there is no single interoperability mechanism that is optimal for every Boost component. A small C-compatible interface is attractive when legacy C++03 compatibility and ABI stability dominate the requirements, whereas a higher-level bridge may provide more expressive interfaces when the C++ baseline permits it. This distinction is directly relevant to the two case studies: Boost.Align targets a C++03-compatible boundary, while the selected Boost.Endian integration uses \texttt{cxx} and therefore has a different compatibility envelope.
...
There are also Rust-native alternatives to several Boost libraries available through the Rust ecosystem. The following table provides examples of such alternatives \parencite{crates_io}.
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|l|l|}
        ...
        \texttt{clap}, \texttt{argh}, \texttt{pico-args} &
        ...
        Boost.Variant &
        \texttt{enum}, \texttt{enum\_dispatch} &
